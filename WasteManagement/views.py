...
def alert_record_view(request):
    ...
    dept_search = dept_search.strip()
    if dept_search:
        configs = configs.filter(department__name__icontains=dept_search).first()
    ...
    paginator_2 = Paginator(dept_list, 10)
    page_obj_2 = paginator_2.get_page(request.GET.get('page2', 1))
...
def save_alert_settings(request):
    try:
        ...
        if waste_type:
            configs = AlertConfig.objects.filter(waste_type_id=waste_type)
            if configs.exists():
                configs.update(weight_max=weight_max, weight_min=weight_min)
            else:
                AlertConfig.objects.create(waste_type_id=waste_type, weight_max=weight_max, weight_min=weight_min)

        if department:
            configs = AlertConfig.objects.filter(department_id=department)
            if configs.exists():
                configs.update(weighting_counts=weighting_counts)
            else:
                AlertConfig.objects.create(department_id=department, weighting_counts=weighting_counts)
...
def last_month_alert_trend(request):
    today = timezone.now().date()
    alert_settings = AlertConfig.objects.filter(is_active=True)
    ...
    qs = (
        WasteRecord_New.objects
        .filter(query, create_time__date__gte=first_day_last_month, create_time__date__lte=last_day_last_month
        )
    ...
    result = {}
    for i in range((last_day_last_month - first_day_last_month).days + 1):
        d = first_day_last_month + timedelta(days=i)
        result[str(d)] = 0

    for item in qs:
        result[str(item['date'])] = item['count']
...
def get_alert_trend_data(request):
    view_type = request.GET.get('type', 'month')
    target_date = request.GET.get('date') # YYYY-MM
    if not target_date:
    ...
    query = Q(transportrecord__isnull=True) & (weight_alert_q | Q(create_time__lte=month_threshold))
    
    if view_type=='year':
        year = target_date.split('-')[0]
        stats = WasteRecord_New.objects.filter(query, create_time__year=year)\
            .annotate(period=TruncMonth('create_time'))\
            .values('period')\
            .annotate(count=Count('id'))\
            .order_by('period')
        labels = [f"{m}月" for m in range(1, 13)]
        data_map = {s['period'].month: s['count'] for s in stats}
        counts = [data_map.get(m, 0) for m in range(1, 13)]
    else:
        parts = target_date.split('-')
        if len(parts) < 2:
            year, month = int(parts[0]), timezone.now().month
        else:
            year, month = int(parts[0]), int(parts[1])
        last_day = calendar.monthrange(year, month)[1]

        stats = WasteRecord_New.objects.filter(query, create_time__year=year, create_time__month=month)\
            .annotate(period=TruncDay('create_time'))\
            .values('period')\
            .annotate(count=Count('id'))\
            .order_by('period')
        data_map = {day: 0 for day in range(1, last_day + 1)}
        for s in stats:
            data_map[s['period'].day] = s['count']
        labels = [f"{day:02d}日" for day in range(1, last_day + 1)]
        counts = [data_map[day] for day in range(1, last_day + 1)]

    return JsonResponse({'labels': labels, 'counts': counts})

def monitor_view(request):
    search_q = request.GET.get('dept_search', '')
    page_number = request.GET.get('page', 1)
    
    yesterday = timezone.now().date() - timedelta(days=1)

    departments = Department.objects.filter(name__icontains=search_q)

    dept_list_all = []
    for dept in departments:
        config = dept.alert_configs.first() 
        standard = config.overdue_hours if config else 1 
        
        # 統計該部門昨日實際過磅次數
        actual = WasteRecord_New.objects.filter(
            department=dept,
            create_time__date=yesterday
        ).count()

        # 計算百分比與狀態
        percent = min(int((actual / standard) * 100), 100) if standard > 0 else 0
        status = 'danger' if actual < standard else 'success'

        dept_list_all.append({
            'name': dept.name,
            'actual': actual,
            'standard': standard,
            'percent': percent,
    ...
    paginator = Paginator(dept_list_all, 10)
    page_obj = paginator.get_page(page_number)

    context = {
        'page_obj': page_obj,
        'dept_list': page_obj.object_list, 
        'search_q': search_q,
    }
    return render(request, 'management/alert_record.html', context)